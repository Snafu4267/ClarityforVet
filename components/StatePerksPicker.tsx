...
/**
 * Pick a state → `/perks/[code]` for that state’s perks quick reference (hub page has national + general notes too).
 */
export function StatePerksPicker() {
  ...
  function onSubmit(e: React.FormEvent) {
    ...
    if (!code) return;
    setPending(true);
    router.push(`/perks/${code.toLowerCase()}`);
  }

  return (
    <form onSubmit={onSubmit} className={bigSurfaceCardClass}>
      <p className="font-semibold text-zinc-900">Jump to your state</p>
      <p className="mt-1 text-sm text-zinc-600">
        Opens that state&apos;s quick list (portal + programs). Not VA hospitals—confirm on official sites.
      </p>
