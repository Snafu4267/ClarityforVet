...
const inputClass =
  "rounded-md border border-stone-300 px-3 py-2.5 text-sm text-stone-900 focus:border-stone-400 focus:outline-none focus:ring-2 focus:ring-stone-200/80";
const primaryBtn =
    ...
    async (e: React.FormEvent) => {
      ...
      if (raw) {
        try {
          ...
          setRegPending(false);
          setRegError("Server returned an invalid response. Try again.");
          return;
        }
      }
      if (!res.ok) {
      ...
      setRegPending(false);
      if (sign?.error) {
        setRegError("Account created but sign-in failed. Try signing in.");
        return;
      }
      router.push("/");
      router.refresh();
    ...
    <ServiceSubpageFrame>
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-xl flex-col gap-10 px-6 py-12 pb-20 sm:gap-12">
        <header className="flex flex-col gap-4">
          ...
          <p className="text-sm leading-relaxed text-stone-700 sm:text-[0.9375rem]">
            Start your free trial on {SITE_NAME}, then sign in and upgrade any time.
          </p>
        </header>

        <section className={cardClass} aria-labelledby="create-account-heading">
          ...
          <form className="mt-5 flex flex-col gap-4" onSubmit={onRegister} noValidate>
            ...
            <div className="flex flex-col gap-1.5 text-sm">
              <label htmlFor={regInviteId} className="text-stone-800">
                Invite code <span className="text-stone-500">(optional)</span>
              </label>
              <input
                id={regInviteId}
                type="text"
                ...
                inputMode="text"
                spellCheck={false}
                placeholder="Their code (letters and numbers)"
                className={inputClass}
